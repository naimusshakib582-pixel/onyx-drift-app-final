...
import { 
  ...
  HiMicrophoneSlash,       // ঠিক করা হয়েছে
  HiVideoCameraSlash,      // ঠিক করা হয়েছে
    ...
    <div className="fixed inset-0 bg-[#050505] z-[3000] flex flex-col p-4 font-sans">
      ...
      <div className="flex justify-between items-center mb-6 px-2">
        <div>
          ...
          <p className="text-white/60 text-sm font-mono">Room: {roomId?.substring(0, 8)}...</p>
        </div>
        <div className="flex items-center gap-2 bg-zinc-900/50 px-3 py-1.5 rounded-full border border-white/5">
          <div className="w-2 h-2 bg-red-500 rounded-full animate-pulse"></div>
          <span className="text-[10px] font-bold text-white">04:22</span>
        </div>
      </div>
          ...
          >
            {peer.id === 'me' ? (
              <video 
                ...
                muted 
                playsInline 
                className={`w-full h-full object-cover scale-x-[-1] ${!videoActive && 'hidden'}`} 
              />
            ) : null}

            {/* ভিডিও বন্ধ থাকলে বা ইউজার অফলাইন থাকলে এই স্ক্রিন দেখাবে */}
                ...
                {!peer.isOnline && peer.id !== 'me' ? (
                   <div className="text-center">
                      <div className="w-16 h-16 bg-zinc-800 rounded-full mx-auto mb-2 animate-pulse flex items-center justify-center">
                         <HiVideoCameraSlash className="text-zinc-600" size={24} />
                      </div>
                  ...
                  <div className="relative">
                    <img 
                      ...
                      alt="avatar"
                    />
                    {!micActive && peer.id === 'me' && (
                      <div className="absolute -bottom-1 -right-1 bg-red-500 p-1 rounded-full border-2 border-zinc-900">
                        <HiMicrophoneSlash size={12} className="text-white" />
                      </div>
                    )}
                  </div>
                )}
      ...
      <div className="fixed bottom-10 left-1/2 -translate-x-1/2 flex items-center gap-6 bg-zinc-900/80 backdrop-blur-3xl px-8 py-5 rounded-[3rem] border border-white/10 shadow-[0_20px_50px_rgba(0,0,0,0.5)]">
        ...
        >
          {micActive ? <HiOutlineMicrophone size={24} /> : <HiMicrophoneSlash size={24} />}
        </button>
        ...
        >
          {videoActive ? <HiOutlineVideoCamera size={24} /> : <HiVideoCameraSlash size={24} />}
        </button>
      </div>
    </div>
  );
