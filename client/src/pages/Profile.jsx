...
import { 
  ...
  FaRocket, FaCamera, FaImage, FaFilm, FaPlayCircle, FaTimes, FaPlus, FaCheckCircle, FaGhost, FaUserPlus, FaEnvelope
} from "react-icons/fa";
...
const Profile = () => {
  ...
  // নিজের প্রোফাইল কি না চেক করার লজিক
  const isOwnProfile = !userId || userId === currentUser?.sub;

  return (
    <div className={`w-full min-h-screen transition-all duration-700 ${isGhostMode ? 'bg-black' : 'bg-[#020617]'} text-gray-200 overflow-x-hidden`}>
      ...
      <div className="max-w-[900px] mx-auto px-4 -mt-16 md:-mt-24 relative z-20">
        ...
        >
          <div className="flex flex-col md:flex-row justify-between items-center md:items-end gap-6">
            ...
            {/* বাটন সেকশন: নিজের প্রোফাইল বনাম অন্য ইউজার */}
            <div className="flex gap-2 md:gap-3 w-full md:w-auto">
              {isOwnProfile ? (
                ...
                <>
                  <button className="flex-1 md:flex-none px-4 md:px-6 py-3 bg-cyan-600 hover:bg-cyan-500 border border-cyan-400/30 rounded-xl md:rounded-2xl text-[9px] md:text-[10px] font-black uppercase tracking-widest text-white transition-all flex items-center justify-center gap-2 shadow-lg shadow-cyan-500/20">
                    <FaUserPlus /> Add Friend
                  </button>
                  <button className="flex-1 md:flex-none px-4 md:px-6 py-3 bg-white/5 hover:bg-white/10 border border-white/10 rounded-xl md:rounded-2xl text-[9px] md:text-[10px] font-black uppercase tracking-widest text-white transition-all flex items-center justify-center gap-2">
                    <FaEnvelope /> Message
                  </button>
                </>
              )}
              ...
              <AnimatePresence>
                {!isGhostMode && (
                  <motion.div 
                    ...
                    className="flex flex-wrap justify-center md:justify-start gap-3 md:gap-5 mt-6 text-[8px] md:text-[9px] font-black text-gray-500 uppercase tracking-widest"
                  >
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[100] flex items-center justify-center p-3 md:p-4 bg-black/95 backdrop-blur-2xl">
            <motion.div initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} className="bg-[#0f172a] w-full max-w-lg rounded-[2.5rem] md:rounded-[3rem] border border-white/10 p-6 md:p-8 shadow-2xl overflow-y-auto max-h-[85vh] no-scrollbar">
              ...
              <div className="space-y-6">
                <div className="space-y-4">
                  <div className="space-y-1">
                    ...
                    <input 
                      type="text" 
                      className="w-full bg-white/5 border border-white/10 rounded-xl md:rounded-2xl p-4 text-base md:text-sm outline-none focus:border-cyan-400 text-white" 
                  ...
                  <div className="space-y-1">
                    ...
                    <textarea 
                      className="w-full bg-white/5 border border-white/10 rounded-xl md:rounded-2xl p-4 text-base md:text-sm outline-none focus:border-cyan-400 h-24 text-white resize-none" 
                      value={editData.bio}
              ...
              <button 
                ...
                disabled={isUpdating}
                className="w-full mt-8 py-4 bg-gradient-to-r from-cyan-500 to-purple-600 rounded-xl md:rounded-2xl font-black text-[10px] md:text-xs uppercase tracking-[0.2em] md:tracking-[0.3em] shadow-lg shadow-cyan-500/20 disabled:opacity-50"
              >
        ...
        {isCreateOpen && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[100] flex items-center justify-center p-3 md:p-4 bg-black/90 backdrop-blur-xl">
            <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} className="bg-[#0f172a] w-full max-w-lg rounded-[2.5rem] md:rounded-[3rem] border border-white/10 p-6 md:p-8 shadow-2xl overflow-y-auto max-h-[85vh]">
              ...
              <textarea 
                className="w-full bg-transparent border-none outline-none text-white placeholder:text-gray-700 text-base md:text-lg mb-6 resize-none h-32" 
