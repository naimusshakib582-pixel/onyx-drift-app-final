...
const Messenger = ({ socket }) => { 
  ...
  return (
    <div className="flex h-screen bg-black text-white overflow-hidden fixed inset-0 font-sans z-[9999]">
        ...
        <div className="p-4 space-y-4">
          <div className="flex justify-between items-center pt-2">
            ...
            <div className="flex gap-2">
              <button className="p-2 bg-zinc-800 rounded-full hover:bg-zinc-700"><HiOutlineCamera size={20}/></button>
              <button className="p-2 bg-zinc-800 rounded-full hover:bg-zinc-700"><HiPlus size={20}/></button>
            </div>
          </div>
          ...
          <div className="flex gap-3 overflow-x-auto no-scrollbar py-2">
            <div className="flex flex-col items-center gap-1 min-w-[65px]">
              <div className="w-14 h-14 rounded-full border border-zinc-700 flex items-center justify-center cursor-pointer bg-zinc-900">
                <HiPlus className="text-white" />
              </div>
              <span className="text-[11px] text-zinc-400">Your Story</span>
            </div>
            {[1,2,3,4,5].map(i => (
              <div key={i} className="flex flex-col items-center gap-1 min-w-[65px]">
                <div className="w-14 h-14 rounded-full border-2 border-blue-600 p-0.5 cursor-pointer">
                  <img src={`https://i.pravatar.cc/150?u=${i+10}`} className="w-full h-full rounded-full object-cover" alt=""/>
                </div>
                <span className="text-[11px] text-zinc-300">User_{i}</span>
              </div>
            ))}
          </div>
        </div>
            ...
            <div key={c._id} onClick={() => setCurrentChat(c)} className={`mx-2 px-3 py-3 rounded-xl flex items-center gap-3 cursor-pointer transition-colors ${currentChat?._id === c._id ? 'bg-zinc-900' : 'hover:bg-zinc-900/50'}`}>
              ...
              <div className="flex-1">
                <div className="flex justify-between">
                   <span className="text-[15px] font-semibold">Node_{c._id.slice(-4)}</span>
                   <span className="text-[12px] text-zinc-500">4:20 PM</span>
                </div>
          ...
          <>
            ...
            <header className="px-4 py-3 flex justify-between items-center border-b border-zinc-800 z-20 bg-black">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-[16px] font-bold leading-none">Node_{currentChat._id.slice(-4)}</h3>
                  <span className="text-[12px] text-zinc-500">Active now</span>
                </div>
              </div>
              <div className="flex gap-4 text-blue-500">
                <button onClick={() => handleCall('voice')} className="p-2"><HiOutlinePhone size={24} /></button>
                <button onClick={() => handleCall('video')} className="p-2"><HiOutlineVideoCamera size={24} /></button>
                <button className="p-2"><HiOutlineInformationCircle size={24} /></button>
              </div>
            </header>
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-3 custom-scrollbar">
              ...
              {isTyping && <div className="text-[11px] text-zinc-500 ml-2">Typing...</div>}
              <div ref={scrollRef} />
            </div>
            ...
            <div className="p-3 flex items-center gap-2 bg-black">
              ...
              {newMessage.trim() ? (
                <button onClick={handleSend} className="text-blue-500 p-1"><HiOutlinePaperAirplane size={24}/></button>
              ) : (
                <button className="text-blue-500 p-1"><HiOutlineMicrophone size={24}/></button>
              )}
            </div>
          </>
        ) : (
      ...
      <AnimatePresence>
        {incomingCall && (
          <div className="fixed inset-0 z-[10000] bg-zinc-900 flex flex-col items-center justify-center text-white">
             ...
             <div className="flex gap-16 mt-32">
                <button onClick={() => { ringtoneRef.current.pause(); setIncomingCall(null); }} className="w-16 h-16 bg-red-500 rounded-full flex items-center justify-center"><HiXMark size={32}/></button>
                <button onClick={() => { ringtoneRef.current.pause(); navigate(`/call/${incomingCall.roomId}`); setIncomingCall(null); }} className="w-16 h-16 bg-green-500 rounded-full flex items-center justify-center animate-bounce"><HiOutlinePhone size={32}/></button>
             </div>
          </div>
        )}
      </AnimatePresence>
